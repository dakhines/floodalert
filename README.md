...
### AI Decision Pipeline & Rules

FloodAlert uses a deterministic backend-first decision pipeline so flood status is based on evidence, not AI guessing. Gemini is used only to summarise and explain the result in clear user-friendly language.

**Source Priority**
1. **JPS Water Level Data:** Primary flood-risk signal and ground truth for river water level status.
2. **Public Infobanjir Alerts/Sirens:** Secondary confirmation when official flood alerts or sirens are available.
3. **METMalaysia Forecasts:** Supporting weather context only. Heavy rain can increase risk, but rain alone does not confirm flooding.
4. **NADMA Notices:** Official disaster or evacuation context when relevant notices are available.
5. **Backend Fallback:** If sources are missing, slow, or incomplete, the backend returns a safe structured fallback instead of crashing the app.

**Flood Status Ladder**
The app only uses these five statuses:
1. `Safe`
2. `Risk Rising`
3. `Warning`
4. `Flood Confirmed`
...
**Gemini Role**
- Gemini summarises the backend-prepared flood context into short, readable text.
- Gemini does not replace the backend flood decision logic.
- Gemini must not invent official warnings, NADMA notices, or flood confirmations.
- If Gemini fails, times out, returns invalid JSON, or exceeds quota, the backend keeps using its deterministic status, action, and reason.

**Validation & Fallback Rules**
- **JPS first:** Water level data is checked before weather or news context.
- **Rain != Flood:** A safe JPS water level should not become `Warning` only because rain is forecast.
- **Strict JSON output:** AI output is parsed and validated before being sent to the frontend.
- **Graceful fallback:** Missing AI or external-source data should never break the dashboard.

**Antigravity Usage Disclosure**
Antigravity was used to review the flood decision workflow, refine Gemini prompt rules, generate scenario-based validation notes, and prepare documentation artifacts for the hackathon submission.
